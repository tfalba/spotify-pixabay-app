...
export default function TracksListsContainer({
  ...
  const [activePanel, setActivePanel] = useState<"search" | "playlists">("search");
  ...
  return (
    <section className="xl:col-span-1 flex min-w-0 flex-col rounded-3xl border border-[amber]/80 p-6 shadow-glow max-h-[max(600px,calc(100vh-10rem))] overflow-hidden">
      <div className="flex items-center gap-3 pb-3">
        ...
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-slate-50">Discover Tracks</h2>
          <p className="text-sm text-slate-400">Search Spotify, view playlists, and set the tone.</p>
        </div>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={clsx(
              ...
              activePanel === tab.key ? "bg-teal text-midnight" : "text-white/60 hover:text-white",
            )}
          >
            {tab.label}
          </button>
      ...
      <div className="min-h-min flex-1 overflow-y-auto pr-1">
        {activePanel === "search" ? (
          ...
          <PlaylistPicker onPick={onPick} onQueueChange={handleQueueChange} onSetTracks={handleSetTracks} />
        )}
      </div>

      {isAuthenticated && tracks.length > 0 && (
          <TrackList tracks={tracks} selectedTrackId={selectedTrackId ?? null} onPick={(track) => onPick(track)} />
      )}
    </section>
  );
}
